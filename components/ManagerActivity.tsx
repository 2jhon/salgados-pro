...
export const ManagerActivity: React.FC<ManagerActivityProps> = ({ 
  ...
  const toggleSelectAll = (items: Transaction[]) => {
    const allIds = items.map(t => String(t.id));
  ...
  return (
    <div className="space-y-6">
                              ...
                              <table className="w-full text-sm text-left">
                                  ...
                                  <tr>
                                    ...
                                    <th className="px-4 py-2 text-center w-10">
                                      <button 
                                        ...
                                        className={`p-1 rounded-md transition-all ${isSelectionMode ? 'bg-rose-500 text-white shadow-lg' : 'hover:bg-slate-200 text-slate-400'}`}
                                      >
                                        <Trash2 size={12} />
                                      </button>
                                    </th>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                  {group.items.map((t) => {
                                    ...
                                    const isExpense = t.subCategory === 'GASTOS';
                                    const isAudit = (t.category || '').trim().toUpperCase() === 'AUDITORIA';
                                    const isItemSelectionMode = isSelectionMode;
                                    const isSelected = selectedIds.has(String(t.id));

                                    return (
                                      <tr key={t.id} className={`hover:bg-slate-50/80 transition-colors ${isAudit ? 'bg-amber-50/30' : ''} ${isSelected ? 'bg-blue-50/50' : ''}`}>
                                        <td className="px-4 py-3 text-slate-400 text-xs font-medium tabular-nums">
                                          {isItemSelectionMode ? (
                                            <input 
                                              type="checkbox" 
                                              checked={isSelected}
                                              onChange={() => toggleSelectOne(String(t.id))}
                                        ...
                                        <td className="px-4 py-3">
                                           <div className="flex flex-col">
                                              ...
                                              {isItemSelectionMode && <span className="sm:hidden text-[10px] text-slate-400 tabular-nums">{time}</span>}
                                           </div>
                                        </td>
                                        ...
                                        <td className="px-4 py-3 text-center">
                                          {isItemSelectionMode ? (
                                            <button 
                                              onClick={() => toggleSelectOne(String(t.id))}
                                              className={`p-1.5 rounded-lg transition-all ${isSelected ? 'text-blue-500 bg-blue-100' : 'text-slate-200'}`}
                                            >
      ...
      {/* Floating Action Button for Bulk Deletion */}
      {isSelectionMode && selectedIds.size > 0 && (
        <div className="fixed bottom-32 left-0 right-0 px-6 z-[100] animate-in slide-in-from-bottom-10 duration-500">
           <div className="max-w-md mx-auto bg-slate-900 text-white p-4 rounded-3xl shadow-2xl border border-slate-700 flex items-center justify-between">
              <div className="flex items-center gap-4">
                 <div className="bg-rose-600 p-2 rounded-xl">
                   <Trash2 className="w-5 h-5" />
                 </div>
                 <div>
                    <h4 className="font-black text-sm uppercase tracking-tight">{selectedIds.size} Itens Selecionados</h4>
                    <p className="text-[10px] font-bold text-slate-400 uppercase">Prepare para excluir em massa</p>
                 </div>
              </div>
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className="px-4 py-2 rounded-xl text-[10px] font-black uppercase text-slate-400 hover:bg-slate-800 transition-all"
                >
                  Cancelar
                </button>
                <button 
